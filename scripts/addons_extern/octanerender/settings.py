...
def addProperties():
    ...
    OctaneRenderSettings.export_mode = EnumProperty(
        items=(
            ("MODE_IMAGE", "Image", "Export still image"),
            ("MODE_ANIMATION", "Animation", "Export animation"),
            ("MODE_GEOMETRY", "Geometry", "Export OBJ/MTL only. Dont start Octane"),
        ),
        name="Panel Mode",
        description="Render/Export target",
        default="MODE_IMAGE")

    OctaneRenderSettings.export_camera_only = BoolProperty(
        name="Camera only",
        description="Export only the active camera",
        default=False)
...
# Octane Project Settings
    ...
    OctaneRenderSettings.export_camera = BoolProperty(
        name="Export camera",
        description="Exports the current active camera settings and writes them down in the selected RT node",
        default=True)

    OctaneRenderSettings.export_resolution = BoolProperty(
        name="Export resolution",
        description="Exports the current resolution from Blender and overwrites the one in Octane",
        default=True)

    OctaneRenderSettings.export_instances = BoolProperty(
        name="Export instances",
        description="Exports all the active particle systems into .csv files",
        default=True)
...
# Octane System settings
    ...
    ###################################################
    # Added by Matej for instance transforms export
    ###################################################
    OctaneRenderSettings.instances_write_dupli = BoolProperty(
        name="Write dupli objects to file",
        description="Will create .obj/.mtl files for the duplication object(s)",
        default=True)

    OctaneRenderSettings.instances_export_mode = EnumProperty(
        name="Export mode",
        items=(('Auto', 'Auto', 'Exports all active particle systems'),
               ('Manual', 'Manual', 'Manually export the selected PS')),
        default='Auto',
    )
    OctaneRenderSettings.node_render_geometry = StringProperty(
        name="Geometry node",
        description="The name of the geometry node to render inside .ocs. If blank, project name will be used.",
        maxlen=128,
        default="")
    OctaneRenderSettings.node_render_target = StringProperty(
        name="Render Target",
        description="The name of the RenderTarget node to render inside .ocs. If set, takes precedence over geometry node",
        maxlen=128,
        default="")
