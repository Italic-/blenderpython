...
def rename_temporary_files(temp_files):
    """
    Will take the temporary files list names and remove the .temp tag.
    If such renamed files already exist, it will first delete them.

    @param temp_files: The temporary file names (next frame data)
    """
    for src in temp_files:
        assert src.endswith(".temp")
        dst = src[:src.rfind(".temp")]
        # first delete the target file, if exists, then rename the temporary one
        if os.path.isfile(dst):
            os.remove(dst)
        os.rename(src, dst)
